...
- 托盘菜单：查看配额（含快捷键提示）、音效开关、音效组（原版 D1/D2 或小黄鸭 Ya1/Ya2）、大小预设（300/440/580）、开机自启、每轮提示、气泡自动收起、设置、随机语句…、本次退出挂件（下次启动 Codex 恢复）、完全退出。
- 随机语句可以改：托盘“随机语句…”或 `--quotes`，一行一条 `权重|文本`（省略权重即 1，范围 1–99），保存后立刻生效；`overlay.json` 里对应 `quotes` 数组，元素是 `{"t":文本,"w":权重}`，空数组会回落到内置 10 条。
